...
class Tests:
    ...
    def testing(self):
        data = self.openINI("hd.ini")   #0 - nazwa pliku, 1 - ilosc powtorzen testu, 2 - dlugosc optymalnej sciezki, 3 - optymalna sciezka

        for i in range (len(data[0])):
            l = self.test(Graph(data[0][i]), data[1][i])
            self.saveResults(data[0][i], l[0], l[1], data[1][i], data[2][i], data[3][i])
            print("Zakonczono testowanie dla pliku: " + data[0][i])
